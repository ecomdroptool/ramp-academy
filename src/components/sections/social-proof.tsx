...
function LogoPlaceholder({ name }: { name: string }) {
  return (
    <div className="flex items-center gap-2 px-6 py-2">
      <div className="h-6 w-6 rounded-md bg-void/5" />
      <span className="text-sm font-medium text-muted-text whitespace-nowrap">
        {name}
      </span>
    </div>
  );
        ...
        >
          {stats.map((stat) => (
            <div key={stat.label} className="text-center">
              ...
              <p className="mt-1 text-sm text-muted-text">{stat.label}</p>
            </div>
          ))}
        </motion.div>
      </div>
